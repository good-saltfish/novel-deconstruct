...
- **细纲/设定 Planner Agent：受限工具循环（#25，ADR-0006）**：新增 `ndecon.agent`——模型通过 JSON 动作协议（不绑定厂商 function calling，DeepSeek/智谱/Ollama 通用）在 12 步上限内自主循环调用 8 个白名单工具：只读 5（L0.5 知识库方法论检索、章节细纲、已确认骨架部件、本书 L1 bigram 检索、开放伏笔台账）+ 写候选 2（`draft_beats`/`draft_setting`，物理上无法确认/删除/改正文/联网）+ `finish`；安全机制含近 4 轮只读空转死循环检测（保留已提交候选）、工具错误截断 200 字回喂自愈、全链路 thought/observation trace 审计。FakePlanner 确定性脚本供离线/CI，OpenAICompatPlanner 复用 #23 面板会话；产物 Beat（场景/角色/冲突三角/情绪/埋收钩）与 SettingEntry（势力/配角/地点/规则/道具/力量体系）一律候选态，面板每章「Agent 规划」入口与「结构化设定库」卡片展示候选与轨迹，用户确认后才并入权威数据（beats 挂章按序覆盖、settings 同名同类型去重追加）。新增 16 个测试（共 155 全绿），浏览器端到端实测两条任务闭环通过。
